...
<style>
/* Map outer — full width, directly under breadcrumb */
.loc-map-outer {
    position: relative;
    width: 100%;
    height: 420px;
...
@media (min-width: 768px) {
    ...
    .loc-overlay { display: none !important; }
}
...
<main>
    ...
    <section style="padding:2.5rem 0 0;">
        ...
        <div id="loc-overlay" class="loc-overlay" onclick="dayanarc_closeAll()"></div>
    </section>

    <!-- ── Contact section ── -->
    <section style="max-width:1440px; margin:0 auto; padding:4rem 1.5rem 5rem;">
...
function dayanarc_closeAll() {
    document.querySelectorAll('.loc-pin-card.open').forEach(function(c) { c.classList.remove('open'); });
    var overlay = document.getElementById('loc-overlay');
    if (overlay) overlay.classList.remove('open');
}

document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') dayanarc_closeAll();
});
</script>
